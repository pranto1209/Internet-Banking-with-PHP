    ...
    <?php 
      ...
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='alert alert-secondary'>You withdraw $row[debit] BDT from your account at $row[date]</div>";
            }
            if ($row['action'] == 'deposit') 
            {
              echo "<div class='alert alert-success'>You deposit $row[credit] BDT in your account at $row[date]</div>";
            }
            if ($row['action'] == 'deduction') 
            {
              echo "<div class='alert alert-danger'>Deduction have been made for $row[debit] BDT from your account at $row[date] in case of $row[other]</div>";
            }
            if ($row['action'] == 'transfer') 
            {
              echo "<div class='alert alert-warning'>Transfer have been made for $row[debit] BDT from your account at $row[date] in Account Number $row[other]</div>";
            }
